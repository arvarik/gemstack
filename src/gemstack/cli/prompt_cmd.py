...
def create(
    ...
    if prompt_file.exists():
        console.print(
            f"[yellow]⚠️  Prompt {name}/{version} already exists. "
            "Use `--force` to overwrite or bump the version.[/yellow]"
        )
...
def bump(
    ...
    if not prompt_dir.exists():
        console.print(
            f"[red]❌ Prompt '{name}' not found. "
            f"Run `gemstack prompt create {name}` to generate it.[/red]"
        )
        raise typer.Exit(code=1)
